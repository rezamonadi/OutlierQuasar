...
def expandClsuter(PointToExpand, PointNeighbors,Cluster,MinPts,eps,Visited,DistanceMatrix,PointClusterNumber,PointClusterNumberIndex):
	...
	for i in PointNeighbors:
		if Visited[i]==0:
			Visited[i]=1
			Neighbors=np.where(DistanceMatrix[i]<eps)
    ...
    for i in range(m):
        if Visited[i]==0:
            ...
            if len(PointNeighbors[0])<MinPts:
                ...
                expandClsuter(dataset[i],PointNeighbors2,Cluster,MinPts,eps,Visited,DistanceMatrix,PointClusterNumber,PointClusterNumberIndex)
...
Epsilon=2.7
MinumumPoints=4
result =DBSCAN(Data[1:1000,:],Epsilon,MinumumPoints)
